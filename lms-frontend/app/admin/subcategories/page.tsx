    ...
    <ProtectedLayout requiredRole="ADMIN">
      <div className="min-h-screen py-24 px-6 md:px-12 w-full max-w-7xl mx-auto text-white">
        ...
        <div className="mb-8 flex flex-wrap gap-4 items-center">
          <span className="text-white/60 font-bold uppercase text-xs tracking-widest">Filter by Exam:</span>
          <div className="flex flex-wrap gap-2">
            <button 
              onClick={() => setSelectedCategoryId("all")}
              className={`px-4 py-2 rounded-full border transition-all ${selectedCategoryId === "all" ? "bg-purple-600 border-purple-400 font-bold" : "bg-white/5 border-white/10 hover:bg-white/10"}`}
            >
              All Exams
            </button>
            {categories.map(cat => (
              <button 
                ...
                className={`px-4 py-2 rounded-full border transition-all ${selectedCategoryId === cat.id.toString() ? "bg-purple-600 border-purple-400 font-bold" : "bg-white/5 border-white/10 hover:bg-white/10"}`}
              >
                {cat.name}
              </button>
            ))}
            ...
            <AnimatePresence>
              {filteredSubCategories.map((sub, index) => (
                <motion.div
                  ...
                  className="w-full flex flex-col md:flex-row items-center gap-6 p-6 rounded-[24px] bg-[rgba(147,51,234,0.05)] border border-[rgba(147,51,234,0.15)] hover:border-[rgba(147,51,234,0.4)] transition-all group"
                >
                  <div className="w-14 h-14 shrink-0 rounded-2xl bg-gradient-to-br from-purple-800 to-indigo-900 border border-purple-500/30 flex items-center justify-center font-black text-xl text-purple-200">
                    {sub.syllabusCode || sub.name.charAt(0)}
                  </div>
                  
                  <div className="flex-1 w-full">
                    <div className="flex items-center gap-3 mb-1">
                      <h3 className="text-[20px] font-[700] text-purple-100">{sub.name}</h3>
                      <span className="px-2 py-0.5 rounded-md bg-purple-500/20 text-purple-400 text-[10px] font-black uppercase tracking-tighter border border-purple-500/30">
                        {sub.categoryName}
                      </span>
                    </div>
                    <p className="text-[14px] font-[500] text-white/50 line-clamp-1">{sub.description}</p>
                  </div>

                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button 
                      onClick={() => handleOpenModal(sub)}
                      className="p-3 rounded-xl bg-white/5 hover:bg-purple-500/20 text-white border border-white/10 transition-colors"
                      title="Edit Subject"
                    >
                    ...
                    <button 
                      ...
                      className="p-3 rounded-xl bg-white/5 hover:bg-red-500/20 text-red-400 border border-white/10 transition-colors"
        ...
        >
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-2">
              <label className="text-sm font-bold text-white/50 ml-1">Description</label>
              <textarea 
                className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-white focus:outline-none focus:border-purple-500/50 transition-colors min-h-[80px]"
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="mt-4 w-full py-4 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 font-bold shadow-lg"
              type="submit"
            >
